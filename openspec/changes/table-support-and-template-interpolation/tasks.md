...
- [x] 2.4 Write unit tests for `apps/web-builder/src/lib/code-exporter.ts` — `cleanText` with tableCtx (item.field → `items[rowIdx]?.field`) and without (plain `data.var`)
- [x] 2.5 Write unit tests for `code-exporter.ts` `extractVariables` — table mock array generation for `loopOver`, mixed table + plain vars
- [x] 2.6 Write unit tests for `code-exporter.ts` `buildExportData` — 3-tier merge: custom > uploaded > AST, no overwrite, invalid JSON
- [x] 2.7 Write unit tests for `code-exporter.ts` table code gen — striped rows, column width calc, border rect after fix
...
- [x] 3.1 Run `pnpm --filter core test` — all tests pass with coverage thresholds (90% lines, 90% functions, 85% branches, 90% statements)
- [x] 3.2 Run `pnpm typecheck` — no TypeScript errors
- [x] 3.3 Verify all spec scenarios: table preview with data, no-data preview row, nested item paths, missing field, system vars, custom var override
